...
# Neo4j connection details - consider loading these from config or environment variables
...
def create_graph(tx, group, attack, region, country, location, target, date, killed, injured, latitude, longitude):
    ...
    query = """
    // Create or merge the terrorist group
    MERGE (g:Group {name: $group})

    // Create or merge the region and country nodes
    MERGE (r:Region {name: $region})
    MERGE (c:Country {name: $country})


    // Create or merge the location (city)
...
try:
    with driver.session() as session:
        for _, row in data.iterrows():
            # Handle date parsing
            if pd.isna(row["iyear"]) or pd.isna(row["imonth"]) or pd.isna(row["iday"]) or \
                    row["imonth"] not in range(1, 13) or row["iday"] < 1:
                date = "unknown"  # Assign "unknown" string here
            else:
                try:
                    date = datetime(int(row["iyear"]), int(row["imonth"]), int(row["iday"]))  # Ensure int casting
                except ValueError:
                    date = "unknown"
